...
const Gallery = () => {
  ...
  return (
    <div id='gallery' className='w-full h-auto bg-gray-950 text-slate-400 flex flex-col items-center justify-start py-8'>
      <div className='flex flex-col justify-center items-center gap-4 p-8 max-w-[1024px]'>
      ...
      <div className='w-full max-w-[1280px] py-8 px-4 flex flex-col gap-4 md:gap-8 justify-center items-center md:px-8 xl:px-0 duration-300'>
              ...
              <div 
                ...
                <div className='bg-gray-950 bg-opacity-70 image-hover-div'>
                  <button
                    ...
                    className='duration-300 hover:border-white hover:text-white border-slate-400 cursor-pointer uppercase px-4 py-2 border rounded-md'>
                      view original size
                  </button>
                </div>
                <img className='object-cover w-full h-full rounded-md duration-300 cursor-pointer' src={item.path} alt={item.name} />
              </div>
            ))
          )}
        </div>

        {
          numOfImages <= filteredImages.length ? 
            <button onClick={() => setNumOfImages(prev => prev + 20)} className='hover:text-white duration-300 my-8 text:sm md:text-lg'>
              see more...
            </button>
            : numOfImages > 20 &&
              <button onClick={() => setNumOfImages(20)} className='hover:text-white duration-300 my-8 text:sm md:text-lg'>
                show less
              </button>
        }
      </div>
